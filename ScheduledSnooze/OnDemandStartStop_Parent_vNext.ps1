...
.EXAMPLE  
	.\OnDemandStartStop_Parent_vNext.ps1 -Resources "ResourcesJSON" -WhatIf "False"
    $ResourcesJSON = '{
    "Action" : "start",
    "Resources" : [
...
[String]$storageTableName = Get-AutomationVariable -Name 'Internal_OnDemandTableStorageName'
...
try
    {  
        ...
        foreach($resource in $resources){
            ...
            $resourceTypeMapping.Add($strVMuri[$strVMuri.Count-1],$strVMuri[$strVMuri.Count-3] + "::" + $strVMuri[$strVMuri.Count-2])
        ...
        $storageODHContext = (Get-AzureRmStorageAccount -ResourceGroupName $tableStorageRG -Name $StorageAccountName).Context

        $tableOnDemandExecutionHistory = Get-AzureStorageTable -Name $storageTableName -Context $storageODHContext
        ...
        {   
            ...
            {  
                ...
                sleep -Seconds 2 
                [String]$partitionKey = $resourceTypeMapping[$VM.Name]
                ...
                [String]$JobID =  $jobStatus.JobId                                    
                $propertyOnDemandExecutionObj = @{"JobId"=$JobID; `
                                                    "RunbookName"=$jobStatus.RunbookName; `
                                                    "Parameters"=$VM.Name + " " + $Action; `
                                                    "OperationType"=$Action; `
                                                    "ResourceId"=$resourceMapping[$VM.Name]; `
                ...
                $res=Add-StorageTableRow -table $tableOnDemandExecutionHistory -partitionKey $partitionKey -rowKey (get-date).ticks -property $propertyOnDemandExecutionObj
            ...
            While($isAllJobCompleted -ne $true)
            {
                $isAllJobCompleted = $true
                foreach($jobdata in $ChildJobs)
                {
                    $jobInfo = Get-AzureRmAutomationJob -Id $jobdata.JobId -ResourceGroupName $aroResourceGroupName -AutomationAccountName $automationAccountName                   
                    $rowOnDemandExecutionData = Get-AzureStorageTableRowByColumnName -table $tableOnDemandExecutionHistory -columnName "JobId" -value $jobInfo.JobId -operator Equal
                    if($rowOnDemandExecutionData.Result -ne $jobInfo.Status)
                    {
                        if($jobInfo.StartTime -ne $null)
                        {
                            $rowOnDemandExecutionData.StartTime = $jobInfo.StartTime
                        }
                        if($jobInfo.EndTime -ne $null)
                        {
                            $rowOnDemandExecutionData.EndTime = $jobInfo.EndTime
                        }
                        $rowOnDemandExecutionData.Result = $jobInfo.Status
                        Update-AzureStorageTableRow -table $tableOnDemandExecutionHistory -entity $rowOnDemandExecutionData                        
                    }
    ...
    {
        $ex = $_.Exception
        Write-Output "Runbook Execution Completed with errors..."
