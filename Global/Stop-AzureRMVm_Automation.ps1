<#
.SYNOPSIS
  This runbook will stop an Azure ARM VM.

.DESCRIPTION
  This runbook will stop an Azure ARM VM (not classic).
  ...
  The input needed is the data to identify which VM to stop.
...
function Do-TheAction
{
    ...
    # Stop the ARM VM  
    Write-Verbose "Stopping the VM '$VmName' in resource group '$RgName'..." -Verbose
    Stop-AzureRmVm -Name $VmName -ResourceGroupName $RgName -AzureRmContext $CurrentContext -Force
    # [OutputType("PSAzureOperationResponse")]
}
# END FUNCTIONS
